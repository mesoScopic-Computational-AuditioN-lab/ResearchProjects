...
if exist([pwd '/loudness/' ppnum '/' ppnum '-rl_loudness.mat'],'file')
    ...
    % load right left loudness values for participant
    load(fullfile( pwd, 'loudness', ppnum, [ppnum '-rl_loudness.mat']),'relloudnes');
else

    % set right left loudness to ones
    relloudnes = [1, 1];
...
pahandle = PsychPortAudio('Open', [], 1, 2, cfg.sound.MEG.samp_rate, cfg.sound.MEG.nrchannels, [], []);
B = cfg.setup.B;
