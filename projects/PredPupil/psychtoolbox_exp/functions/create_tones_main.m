...
%[pres_freq, cfg.stim_t, cfg.sound.samp_rate, cfg.ramp_ops, cfg.ramp_dns, cfg.n_runs]
...
raw_tones = nan(size(stimuli,1), size(stimuli,2), size(stimuli,3),  size(stim(stimuli(1)),2));  % tones whitout any modulation
mod_tones = nan(size(stimuli,1), size(stimuli,2), size(stimuli,3),  size(stim(stimuli(1)),2));  % tones with rampup/down
...
%% Main loop
for run = 1:size(raw_tones,1)
for block = 1:size(raw_tones,2)
    % get main frequency array
    cur_tones               = stim(stimuli(run,block,:));           % get raw tone     
    raw_tones(run,block, :, :)  = cur_tones;                        % put into matrix
    mod_tones(run,block, :, :)  = cur_tones.*w_on_xt.*w_off_xt;     % apply ramp
end
end
