...
def resample_plot(tr_df, stim_df, coi, timing_column='timing', timing=150):
    """plot down-sampling results as compared to mean of transformation
    input tr_df : dataframe in tr domain
          stim_df : dataframe in original stimulus domain
          coi : collumn of interest to plot (e.g. 'surprisal')
          timing_column : default('timing') column with timing information in both dataframes
          timing : default(150) how long to plot after onset"""

    # Filter data to include only the first 20 seconds
    tr_df_filtered = tr_df[tr_df['timing'] <= tr_df['timing'].min() + 150]
    stim_df_filtered = stim_df[stim_df['timing'] <= stim_df['timing'].min() + 150]
    ...
    # Resampling stim_df to match tr_df timing
    stim_df_resampled = np.interp(tr_df['timing'], stim_df['timing'], stim_df[coi])
    correlation_raw = np.corrcoef(tr_df[coi], stim_df_resampled)[0, 1]
    correlation_resampled = np.corrcoef(tr_df[f'{coi}_resampled'], stim_df_resampled)[0, 1]

    # Plotting the first line with orange color, thicker linewidth, and alpha of 0.5
    ax.plot(tr_df_filtered['timing'], tr_df_filtered[coi], color='darkorange', 
            linewidth=2, alpha=0.5, label=f'Raw Surprisal (corr to baseline:{correlation_raw:.4f})')

    # Plotting the second line with dark orange color, thicker linewidth, and alpha of 0.5
    ax.plot(tr_df_filtered['timing'], tr_df_filtered[f'{coi}_resampled'], color='red', 
            linewidth=2, alpha=0.5, label=f'Resampled Surprisal (corr to baseline:{correlation_resampled:.4f})')

    # Plotting the third line with black color, dotted line style, and a label
    ax.plot(stim_df_filtered['timing'], stim_df_filtered[coi], color='black', linestyle=':', label='Baseline')

    # Adding the legend to the plot
    ax.legend(fontsize=14)
    ax.set_xlabel('CLK (s)', fontsize=16)
    ax.set_ylabel('Magnitude', fontsize=16)
    ax.set_title(coi, fontsize=22)
